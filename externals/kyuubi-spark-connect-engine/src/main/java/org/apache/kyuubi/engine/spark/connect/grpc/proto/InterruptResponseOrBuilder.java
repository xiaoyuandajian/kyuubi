// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: kyuubi/spark/connect/base.proto

package org.apache.kyuubi.engine.spark.connect.grpc.proto;

public interface InterruptResponseOrBuilder
    extends
    // @@protoc_insertion_point(interface_extends:spark.connect.InterruptResponse)
    com.google.protobuf.MessageOrBuilder {

  /**
   *
   *
   * <pre>
   * Session id in which the interrupt was running.
   * </pre>
   *
   * <code>string session_id = 1;</code>
   *
   * @return The sessionId.
   */
  java.lang.String getSessionId();
  /**
   *
   *
   * <pre>
   * Session id in which the interrupt was running.
   * </pre>
   *
   * <code>string session_id = 1;</code>
   *
   * @return The bytes for sessionId.
   */
  com.google.protobuf.ByteString getSessionIdBytes();

  /**
   *
   *
   * <pre>
   * Server-side generated idempotency key that the client can use to assert that the server side
   * session has not changed.
   * </pre>
   *
   * <code>string server_side_session_id = 3;</code>
   *
   * @return The serverSideSessionId.
   */
  java.lang.String getServerSideSessionId();
  /**
   *
   *
   * <pre>
   * Server-side generated idempotency key that the client can use to assert that the server side
   * session has not changed.
   * </pre>
   *
   * <code>string server_side_session_id = 3;</code>
   *
   * @return The bytes for serverSideSessionId.
   */
  com.google.protobuf.ByteString getServerSideSessionIdBytes();

  /**
   *
   *
   * <pre>
   * Operation ids of the executions which were interrupted.
   * </pre>
   *
   * <code>repeated string interrupted_ids = 2;</code>
   *
   * @return A list containing the interruptedIds.
   */
  java.util.List<java.lang.String> getInterruptedIdsList();
  /**
   *
   *
   * <pre>
   * Operation ids of the executions which were interrupted.
   * </pre>
   *
   * <code>repeated string interrupted_ids = 2;</code>
   *
   * @return The count of interruptedIds.
   */
  int getInterruptedIdsCount();
  /**
   *
   *
   * <pre>
   * Operation ids of the executions which were interrupted.
   * </pre>
   *
   * <code>repeated string interrupted_ids = 2;</code>
   *
   * @param index The index of the element to return.
   * @return The interruptedIds at the given index.
   */
  java.lang.String getInterruptedIds(int index);
  /**
   *
   *
   * <pre>
   * Operation ids of the executions which were interrupted.
   * </pre>
   *
   * <code>repeated string interrupted_ids = 2;</code>
   *
   * @param index The index of the value to return.
   * @return The bytes of the interruptedIds at the given index.
   */
  com.google.protobuf.ByteString getInterruptedIdsBytes(int index);
}
